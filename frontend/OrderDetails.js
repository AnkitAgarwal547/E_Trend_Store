                  ...
                  {order?.product?.images.map(image => (
                    <div key={image._id}>
                      <h3 style={contentStyle}>
                        <SideBySideMagnifier style={{ height: "100%", width: "100%" }} alwaysInPlace={true} imageSrc={`${process.env.REACT_APP_SERVER_URL}/uploads/${image.medium}`} imageAlt="Example" />
                      </h3>
                    </div>
                  ))}
                </Carousel>
            ...
            <div className="row mb-4">
              <div className="col-md-6">
                <strong>
                  <p>
                    OrderID
                    <br /> Current Status <br /> Quantity <br /> Created At <br />
                  </p>
                </strong>
              </div>
              <div className="col-md-6 text-md-right">
              ...
              <div className="col-md-6">
                <strong>
                  <p>
                    Transaction Code
                    <br />
                    isPaid <br /> Method <br /> Shipping Charge <br /> Amount <br /> Returned Amount <br />
                  </p>
                </strong>
              </div>
              <div className="col-md-6 text-md-right">
                <p>
                  ...
                  <br />
                  {order && (order?.isPaid ? <span className="badge badge-pill badge-success">yes</span> : <span className="badge badge-pill badge-danger">no</span>)}
              ...
              <div className="col-md-6">
                <strong>
                  <p>
                    Name
                    <br />
                    Email <br /> DOB <br /> Gender <br /> Photo <br />
                  </p>
                </strong>
              </div>
              <div className="col-md-6 text-md-right">
                <p>
                  {order?.user?.name}
                  <br />
                  {order?.user?.email} <br /> {order?.user?.dob}
                  <br /> {order?.user?.gender}
                  <br /> {<Avatar shape="square" size="medium" src={`${process.env.REACT_APP_SERVER_URL}/uploads/${order?.user.photo}`} />}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="card">
          <div className="card-body">
            ...
            <ul className="timeline mt-2 mb-0">
              ...
              {order?.status?.dispatchedDetail?.dispatchedDate && (
                <li className="timeline-item">
                  <strong>Dispatched</strong>
                  <span className="float-right text-muted text-sm">{moment(order?.status?.dispatchedDetail?.dispatchedDate).fromNow()}</span>
                  <small>
                    <p>
                      This order is dispatched by {order?.status?.dispatchedDetail?.dispatchedBy?.name}
                      <br />
                      <strong>Email: </strong> {order?.status?.dispatchedDetail?.dispatchedBy?.email}
                      <br />
              ...
              {order?.status?.returnedDetail?.returnedDate && (
                <li className="timeline-item">
                  <strong>Returned</strong>
                  <span className="float-right text-muted text-sm">{moment(order?.status?.returnedDetail?.returnedDate).fromNow()}</span>
                  <small>
                    <p>
                      This order is returned by {order?.status?.returnedDetail?.returneddBy?.name}
                      <br />
                      <strong>Email: </strong> {order?.status?.returnedDetail?.returneddBy?.email}
                      <br />
                      <strong>Phone: </strong> {order?.status?.returnedDetail?.returneddBy?.phone}
                      <br />
                      <strong>Customer reason: </strong> {order?.status?.returnedDetail?.remark[0]?.comment}
                      <br />
                    </p>
                  </small>
                </li>
              ...
              {order?.status?.cancelledDetail?.cancelledDate && (
                <li className="timeline-item">
                  ...
                  <small>
                    {" "}
                    <p>
                      This order is cancelled by {order?.status?.cancelledDetail?.cancelledBy?.name}.<br />
                      <strong>Reason: </strong>
                      {order?.status?.cancelledDetail?.remark?.comment}
                    </p>
                  </small>
                </li>
              )}
